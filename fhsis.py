...
def extract_period(filename):
    # Searches for common month/quarter markers in filename
...
def parse_fhsis_template(uploaded_file):
    ...
    if area_col is None: return None, "Area Col Not Found"
    ...
    clean_df = df.iloc[int(abra_idx):].copy()
...
st.markdown(f"### DOH 8-Point Action Agenda Monitoring")

with st.expander(f"📂 Upload {target_year} FHSIS Templates", expanded=True):
    files = st.file_uploader("Upload CSV/Excel Files", accept_multiple_files=True)
...
if files:
    for f in files:
        ...
        data, status = parse_fhsis_template(f)
        if data is not None:
    ...
    st.success(f"Successfully processed {len(files)} files into the database.")
...
if not db.empty:
    ...
    for i, tab_name in enumerate(PRIORITY_AGENDA):
        with tabs[i]:
            ...
            sel_metric = col1.selectbox("Select Metric:", metrics, key=f"met_{i}")
            sel_munis = col2.multiselect("Compare RHUs:", ABRA_MUNIS, default=['Bangued', 'Manabo'], key=f"mun_{i}")
            
            trend_df = tab_data[(tab_data['Metric'] == sel_metric) & (tab_data['Municipality'].isin(sel_munis))]
    ...
    st.warning("Please upload FHSIS files to populate the Command Center.")
