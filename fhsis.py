...
def load_and_clean_mortality_data(uploaded_file, year):
    try:
        ...
        is_traffic_death = "plus 2" in name_low or "traffic injuries" in name_low
        is_traffic_acc = "plus 3" in name_low or "traffic accidents" in name_low
        ...
        for month_val, df in sheets_to_process.items():
            ...
            # Target columns by absolute integer index
            if is_premature_ncd:
                # Area(0), Total Deaths(4), CVD(8), Cancer(12), Diabetes(16), Respiratory(20)
                cols_to_keep = [0, 4, 8, 12, 16, 20]
                col_names = ['Area', 'Total Premature Deaths', 'CVD Deaths', 'Cancer Deaths', 'Diabetes Deaths', 'Respiratory Disease Deaths']
            elif is_traffic_death:
                # Area(0), Total Traffic Deaths(4)
                cols_to_keep = [0, 4]
                col_names = ['Area', 'Traffic Injury Deaths']
            elif is_traffic_acc:
                # Area(0), Total Road Accidents(1)
                cols_to_keep = [0, 1]
                col_names = ['Area', 'Total Road Accidents']
            else:
                continue 
            ...
            clean = pd.DataFrame()
            clean['Matched_RHU'] = df[0].apply(extract_rhu)
            
            mapped_df = pd.DataFrame()
            mapped_df['Area'] = clean['Matched_RHU']
            
            for i, col_idx in enumerate(cols_to_keep[1:], 1):
                if col_idx < df.shape[1]:
                    mapped_df[col_names[i]] = pd.to_numeric(df[col_idx], errors='coerce').fillna(0)
                else:
